...
int main(int argc, char ** argv) {
    ...
    params.warmup = false; // K3 warmup activates all 896 experts — skip it

    // self-draft uses the standalone draft-model impl (driven by our ctx_dft); enable it explicitly
    // since we don't pass -md (which would normally set this), leaving types at NONE.
    ...
    // SELF-DRAFT: reuse the TARGET model (one copy of the weights) as the draft, with a reduced
    // number of active experts (LLAMA_DRAFT_N_EXPERT, default 2). No second model is loaded —
    // 2x the weights would not fit RAM. The draft context shares model_tgt via the per-context
    // n_expert_used override; the tested spec-decode loop below is otherwise unchanged.
    llama_context_ptr ctx_dft;
    {
        const int draft_n_expert = getenv("LLAMA_DRAFT_N_EXPERT") ? atoi(getenv("LLAMA_DRAFT_N_EXPERT")) : 2;

        auto cparams = common_context_params_to_llama(params);
        cparams.n_expert_used = (uint32_t) draft_n_expert;
        ctx_dft.reset(llama_init_from_model(model_tgt, cparams));
        if (ctx_dft == nullptr) {
            LOG_ERR("failed to create self-draft context (n_expert_used=%d)\n", draft_n_expert);
            return 1;
        }
        LOG_INF("self-draft: reusing target model with n_expert_used=%d\n", draft_n_expert);
